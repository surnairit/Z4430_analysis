...
#include "RecoVertex/KinematicFit/interface/MultiTrackMassKinematicConstraint.h" //nsmod
...
const ParticleMass B0_mass = 5.27961; //pdg mass
...
MuMuPiKPAT::MuMuPiKPAT(const edm::ParameterSet& iConfig) :
  ...
  doMC( iConfig.getUntrackedParameter<bool>("DoMonteCarloTree", false) ), //true //nsmod
  ...
  doB0MassConst( iConfig.getUntrackedParameter<bool>("DoB0MassConstraint", true) ), //nsmod

  skipJPsi(iConfig.getUntrackedParameter<bool>("SkipJPsi", false)),
  skipPsi2S(iConfig.getUntrackedParameter<bool>("SkipPsi2S", true)), // false //nsmod
		  ...
		  if (doMuMuMassConst) { // MassConst = 'MC' in the following
		    ...
		    if (dimuonType == 1) { // constrain to JPsi mass
		      ...
		      MuMu = new TwoTrackMassKinematicConstraint(psi2S_mass);
                ...
                B0VertexFitTree_noKrefit = B0Fitter.fit( b0Daughters, MuMuPiK );
                }
		  } 
		  else {
