...
//  TFile *f0 = new TFile("MuMuPiK_Jpsi_output_new_comb_B0Pt.root");
      TFile *f0 = new TFile("MuMuPiK_Jpsi_output_new_comb_ext_B0Pt.root");
  ofstream outfile;
  outfile.open("purity_ext_B0Pt.txt");
...
//	double cut=1.8+k/10.0; 
	...
	sprintf(datahist,"dh%i",j);
        sprintf(histname,"hjpsiKPiMassSelAltZoom_B0Pt%i",j+10);
...
//  gr->GetXaxis()->SetTitle("ChiSquared/NDoF cut ");
  ...
  c_purity->SaveAs("purity_ext_B0Pt.pdf");
