...
void psiPrimePiK_MC::SlaveBegin(TTree * /*tree*/)
{
  ...
  OutFile = 0;
  if ( MC ) {
    //OutFile = new TProofOutputFile( option+"_Bd2Psi2SKpi.root" ); // comment this line to not create this file
    OutFile = new TProofOutputFile( option+"_Bd2MuMuKpi_"+B0limits+".root" ); // comment this line to not create this file
  } else {
  ...
  mva_variable_sig = new TTree("mva_variable_sig","MVA signal variables");
  mva_variable_bkg = new TTree("mva_variable_bkg","MVA background variables");
  mva_variable_all = new TTree("mva_variable_all","MVA variables");
  ...
  mva_vars_name.push_back("KPi_mass");
  mva_vars_name.push_back("psiDeltaRHigherPtTrk"); mva_vars_name.push_back("psiDeltaRLowerPtTrk"); mva_vars_name.push_back("pionDeltaRKaon");
  ...
  mva_vars_name.push_back("MuMuPi_mass"); mva_vars_name.push_back("cosTheta_psi"); mva_vars_name.push_back("phi");
  // reflection check
  ...
  for (Int_t i = 0; i < mva_vars_n; ++i) {
    ...
    if ( kTRUE ) {
      mva_variable_sig->Branch(mva_vars_name[i], &mva_vars_sig[i], mva_vars_name[i]+"/F" );
      mva_variable_bkg->Branch(mva_vars_name[i], &mva_vars_bkg[i], mva_vars_name[i]+"/F" );
      mva_variable_all->Branch(mva_vars_name[i], &mva_vars_all[i], mva_vars_name[i]+"/F" );
      //mva_variable_sig->Branch(mva_vars_name[i], &mva_vars_sig[i] );
      //mva_variable_bkg->Branch(mva_vars_name[i], &mva_vars_bkg[i] );
      //mva_variable_all->Branch(mva_vars_name[i], &mva_vars_all[i] );
    } else {
      mva_variable_sig->Branch(mva_vars_name[i], &mva_vars_sig[i], mva_vars_name[i]+"/I" );
      mva_variable_bkg->Branch(mva_vars_name[i], &mva_vars_bkg[i], mva_vars_name[i]+"/I" );
      mva_variable_all->Branch(mva_vars_name[i], &mva_vars_all[i], mva_vars_name[i]+"/I" );
    }
  }
  // tight cuts
  // single
  ...
  cos_psi2S_helicityAngle = new TH1F("cos_psi2S_helicityAngle","cosine of "+mumu_label+" helicity angle reco;cos #theta_{"+mumu_label+"}", 102, -1.02, 1.02) ;
  cos_psi2S_helicityAngleLabB0peak = new TH1F("cos_psi2S_helicityAngleLabB0peak","cosine of "+mumu_label+" lab helicity angle reco in B^{0} peak;cos #theta_{"+mumu_label+"}", 102, -1.02, 1.02) ;
  cos_psi2S_helicityAngleLabB0sb = new TH1F("cos_psi2S_helicityAngleLabB0sb","cosine of "+mumu_label+" lab helicity angle reco in B^{0} sidebands;cos #theta_{"+mumu_label+"}", 102, -1.02, 1.02) ;
  cos_psi2S_helicityAngle_rot = new TH1F("cos_psi2S_helicityAngle_rot","cosine of "+mumu_label+" helicity angle with rotation;cos #theta_{"+mumu_label+"} rot", 102, -1.02, 1.02) ;
  cos_psi2S_helicityAngle_rotVSnotRot = new TH2F("cos_psi2S_helicityAngle_rotVSnotRot","cosine of "+mumu_label+" helicity angle rot VS notRot;cos #theta_{"+mumu_label+"} not rot;cos #theta_{"+mumu_label+"} rot", 102,-1.02,1.02, 102,-1.02,1.02) ;
  ...
  //
  cos_Kstar_helicityAngle = new TH1F("cos_Kstar_helicityAngle","cosine of K* helicity angle reco;cos #theta_{K*}", 102, -1.02, 1.02) ;
  cosKstar_vs_cosPsi2S_helicityAngles = new TH2F("cosKstar_vs_cosPsi2S_helicityAngles","cosine of K* vs cosine of "+mumu_label+" helicity angles;cos #theta_{"+mumu_label+"};cos #theta_{K*}", 102/2,-1.02,1.02, 102/2,-1.02,1.02) ;
  planesAngle = new TH1F("planesAngle","angle between K#pi and #mu#mu planes reco;#phi("+mumu_label+",K*)", phi_bins, -phi_limit, phi_limit) ; 
  planesAngle_vs_cos_psi2S_helicityAngle = new TH2F("planesAngle_vs_cos_psi2S_helicityAngle_1B0","K#pi-#mu#mu planes angle vs cos("+mumu_label+" helicity angle) reco;cos #theta_{"+mumu_label+"};#phi("+mumu_label+",K*)", 102/2,-1.02,1.02, phi_bins/2,-phi_limit,phi_limit) ;
  //
  ...
  psi2SPiSqMass_vs_KPiSqMass_BT_gen = new TH2F("gen_psi2SPi_vs_KPi_dalitz_BT",""+mumu_label+"#pi^{-} vs K^{+}#pi^{-} GEN before trigger;m^{2}(K^{+}#pi^{-});m^{2}("+mumu_label+"#pi^{-})", KPiMass2_bins,KPiMass2_low,KPiMass2_high, MuMuPiMass2_bins,MuMuPiMass2_low,MuMuPiMass2_high) ;
  ...
  //
  psi2SPiSqMass_vs_KPiSqMass_gen = new TH2F("gen_psi2SPi_vs_KPi_dalitz",""+mumu_label+"#pi^{-} vs K^{+}#pi^{-} GEN;m^{2}(K^{+}#pi^{-});m^{2}("+mumu_label+"#pi^{-})", KPiMass2_bins,KPiMass2_low,KPiMass2_high, MuMuPiMass2_bins,MuMuPiMass2_low,MuMuPiMass2_high) ;
  ...
  cos_psi2S_helicityAngle_BT_gen = new TH1F("cos_psi2S_helicityAngle_BT_gen","cosine of "+mumu_label+" helicity angle GEN;cos #theta_{"+mumu_label+"}", 102, -1.02, 1.02) ;
  cos_psi2S_helicityAngle_BT_gen_bias = new TH1F("cos_psi2S_helicityAngle_BT_gen_bias","cosine of "+mumu_label+" helicity angle GEN biased;cos #theta_{"+mumu_label+"}", 102, -1.02, 1.02) ;
  cos_psi2S_helicityAngle_BT_gen_rot = new TH1F("cos_psi2S_helicityAngle_BT_gen_rot","cosine of "+mumu_label+" helicity angle GEN with rotation;cos #theta_{"+mumu_label+"} rot", 102, -1.02, 1.02) ;
  cos_psi2S_helicityAngle_BT_gen_rotVSnotRot = new TH2F("cos_psi2S_helicityAngle_BT_gen_rotVSnotRot","cosine of "+mumu_label+" helicity angle GEN rot VS notRot;cos #theta_{"+mumu_label+"} not rot;cos #theta_{"+mumu_label+"} rot", 102,-1.02,1.02, 102,-1.02,1.02) ;
  cos_psi2S_helicityAngle_vs_psi2SPiMass_BT_gen = new TH2F("cos_psi2S_helicityAngle_vs_psi2SPi_BT_gen","cosine of "+mumu_label+" helicity angle vs m("+mumu_label+"#pi^{-}) GEN;m("+mumu_label+"#pi^{-});cos #theta_{"+mumu_label+"}", 2*24,3.8,4.8, 102,-1.02,1.02) ;
  ...
  cos_Kstar_helicityAngle_BT_gen = new TH1F("cos_Kstar_helicityAngle_BT_gen","cosine of K* helicity angle GEN;cos #theta_{K*}", 102, -1.02, 1.02) ;
  cos_Kstar_helicityAngle_BT_gen_fromMasses = new TH1F("cos_Kstar_helicityAngle_BT_gen_fromMasses","cosine of K* helicity angle GEN from masses;cos #theta_{K*}", 102, -1.02, 1.02) ;
  cosKstar_vs_cosPsi2S_helicityAngles_BT_gen = new TH2F("cosKstar_vs_cosPsi2S_helicityAngles_BT_gen","cosine of K* vs cosine of "+mumu_label+" helicity angles GEN;cos #theta_{"+mumu_label+"};cos #theta_{K*}", 102/2,-1.02,1.02, 102/2,-1.02,1.02) ;
  ...
  planesAngle_vs_cos_psi2S_helicityAngle_BT_gen = new TH2F("planesAngle_vs_cos_psi2S_helicityAngle_BT_gen","K#pi-#mu#mu planes angle vs cos("+mumu_label+" helicity angle) GEN;cos #theta_{"+mumu_label+"};#phi("+mumu_label+",K*)", 102/2,-1.02,1.02, phi_bins/2,-phi_limit,phi_limit) ;
  //
  ...
  */
  ...
  if ( MC ) {
	...
	if (fabs(muPp4_gen.Eta()) < 2.5  &&  fabs(muMp4_gen.Eta()) < 2.5  &&  fabs(Kp4_gen.Eta()) < 2.5  &&  fabs(Pip4_gen.Eta()) < 2.5)
      ...
      Double_t cosTheta_psi_gen = TMath::Cos(theta_psi_gen);
      ...
      //
      cos_psi2S_helicityAngle_vs_psi2SPiMass_BT_gen->Fill((Jp4_gen + Pip4_gen).M(), cosTheta_psi_gen) ;
      cos_psi2S_helicityAngle_vs_KPiMass_BT_gen->Fill((Kp4_gen + Pip4_gen).M(), cosTheta_psi_gen) ;
      //
	...
	return kFALSE;
      ...
      Double_t theta_Kstar_gen = GetThetaMuMu(B0p4_gen, Kstar_gen, Kp4_gen, beam_energy, (Kp4_gen+Pip4_gen).M(), Kp4_gen.M()) ;
	...
	planesAngle_BT_gen_bias->Fill( phi_gen ) ;
      ...
      Double_t NT_var[] = {(Float_t)runNum, (Float_t)evtNum, (*MCkaonCh)[myMCB0Idx] > 0 ? +1. : -1., Kstar_gen.M(), (Jp4_gen + Pip4_gen).M(), cosTheta_psi_gen, phi_gen};
      _nt->Fill( NT_var );
    }
  }
	...
	*/
	...
	if ( 1 
	     ...
	     )
	  {
		...
		Float_t mva_values[] = { (Float_t)(Pip4_orig.Pt()), (Float_t)(Kp4_orig.Pt()), B0_Pt, (Float_t)(Pip4_orig.Pt()), (Float_t)(Kp4_orig.Pt()), (Float_t)(Pip4_orig.Pt() + Kp4_orig.Pt()), (Float_t)(Pip4_orig + Kp4_orig).M(), (Float_t)(PsiPp4_orig.DeltaR(Pip4_orig)), (Float_t)(PsiPp4_orig.DeltaR(Kp4_orig)), (Float_t)(Pip4_orig.DeltaR(Kp4_orig)), (*B0Vtx_CL)[myB0Idx], (Float_t)((*B0CTauPV)[myB0Idx]/(*B0CTauPVE)[myB0Idx]) //, (Float_t)((*B0CTauBS)[myB0Idx]/(*B0CTauBSE)[myB0Idx]) 
					 , (Float_t)((*B0CosAlphaPV)[myB0Idx]), myB0MassAlt, (Float_t)nB0AC
					 , (Float_t)(Zp4_orig.M()), (Float_t)cosTheta_psi, (Float_t)phi // AA vars
		...
		*/
		...
		if (Pip4_orig.Pt() < Kp4_orig.Pt()) {
		  mva_vars_all[3] = Kp4_orig.Pt(); mva_vars_all[4] = Pip4_orig.Pt();
		  mva_vars_all[7] = PsiPp4_orig.DeltaR(Kp4_orig); mva_vars_all[8] = PsiPp4_orig.DeltaR(Pip4_orig); 
		}
		...
		if ( fabs(myB0MassAlt - B0_massFit) < B0_signal*B0_sigmaFit ) { 
		  ...
		  hPionDeltaRKaon_fromB0peak->Fill((Float_t)(Pip4_orig.DeltaR(Kp4_orig))); KPiMass_fromB0peak->Fill( KPip4_orig.M() );
		  if (Pip4_orig.Pt() < Kp4_orig.Pt()) {
		  ...
		  nB0AC_sbWin++;
		  hPionPt_fromB0sb->Fill(Pip4_orig.Pt()); hKaonPt_fromB0sb->Fill(Kp4_orig.Pt()); hB0Pt_fromB0sb->Fill(B0_Pt);
		  if (Pip4_orig.Pt() < Kp4_orig.Pt()) {
		  ...
		  mva_variable_bkg->Fill();
		}
		mva_variable_all->Fill();
...
void psiPrimePiK_MC::SlaveTerminate()
{
  ...
  if (fOut) 
    {      
      ...
      //
      _nt->Write();
      //mva_variable_sig->Write(); mva_variable_bkg->Write(); // causes the same trees in an other output file to have all of the branches at 0
	...
	dEdx_vs_pT[i]->Write() ;
      ...
      hPionPt_fromB0peak->Write() ; hKaonPt_fromB0peak->Write() ; hPionPtPlusKaonPt_fromB0peak->Write(); hB0Pt_fromB0peak->Write(); hHigherPt_fromB0peak->Write(); hLowerPt_fromB0peak->Write(); hPsiDeltaRHigherPtTrk_fromB0peak->Write(); hPsiDeltaRLowerPtTrk_fromB0peak->Write(); hPionDeltaRKaon_fromB0peak->Write(); KPiMass_fromB0peak->Write();
      hHigherPt_fromB0peak->Write(); hB0VtxCL_fromB0peak->Write(); B0_PVCTau_fromB0peak->Write() ; B0_BSCTau_fromB0peak->Write() ; B0_pointingAnglePV_fromB0peak->Write();
      cos_psi2S_helicityAngleLabB0peak->Write();
      hPionPt_fromB0sb->Write() ; hKaonPt_fromB0sb->Write() ; hB0Pt_fromB0sb->Write(); hLowerPt_fromB0sb->Write(); hHigherPt_fromB0sb->Write();
      hB0VtxCL_fromB0sb->Write(); B0_PVCTau_fromB0sb->Write() ; B0_BSCTau_fromB0sb->Write() ; B0_pointingAnglePV_fromB0sb->Write();
      cos_psi2S_helicityAngleLabB0sb->Write();
      //
      // MC analysis
      nMCB0_BT_h->Write() ;
  ...
  if ( tightCutsOut ) 
    {      
      tightCutsOut->cd();
      //
      mva_variable_sig->Write(); mva_variable_bkg->Write(); mva_variable_all->Write();
      //
      kaonP_vs_pionP->Write() ; kaonPt_vs_pionPt->Write() ; kaonIdx_vs_pionIdx->Write() ;
...
void psiPrimePiK_MC::Terminate()
{
  ...
  if ( (OutFile = dynamic_cast<TProofOutputFile*>(fOutput->FindObject( fileName.Data() ))) ) {
	...
	if ( doGEN_plots) {
	  if ( (hB0Mass_1B0matched[0][0] = (TH1F*) AlexisOut->Get("B0Mass_1B0matched")) ) {
	    ...
	    if ( (hB0Mass_1B0matched[0][1] = (TH1F*) AlexisOut->Get("B0Mass_1B0matched_dauCh")) ) {
	      ...
	      gPad->SaveAs( TString::Format("%s/B0mass_1B0.png", dir.Data()) );
	...
	if ( option.Contains("MC")  &&  nEntries.getVal() > 20000 ) twoGauss = kTRUE;

	Double_t fitMax = xMax, fitMin = xMin ;

	RooRealVar mean("m", "mean", 5.28, 5.25, 5.31) ;
	Float_t sigmaThr = 0.03;
	...
	if ( (dalitzGen_squared = (TH2F*) AlexisOut->Get( nameGen_squared.Data() )) ) {
      ...
      //nameGenReco.push_back( make_pair( make_pair("gen_psi2SPi_vs_KPi_dalitz","reco_psi2SPi_vs_KPi_dalitz"), "psi2SPi_vs_KPi") );
      nameGenReco.push_back( make_pair( make_pair("gen_psi2SPi_vs_KPi_dalitz_BT","psi2SPi_vs_KPi_dalitz_1B0"), "psi2SPi_vs_KPi") );
      nameGenReco.push_back( make_pair( make_pair("gen_psi2SPi_vs_KPi_dalitz_BT","psi2SPi_vs_KPi_dalitz_B0constr_1B0"), "psi2SPi_vs_KPi_B0constr") );
