...
using namespace RooFit ;
B0_fit_cut_opt_MuMuVtx_CL()
{ // main
//  TCanvas* c = new TCanvas("c","test",570,400);
//  TFile *f0 = new TFile("psi2spipi_cut_opt_trial.root");

  TFile *f0 = new TFile("MuMuPiK_Jpsi_MuOnia2012ABCD_output_small_ntpls_cut_opt_MuMuVtx_CL.root");
  ofstream outfile;
  outfile.open("purity_MuOnia2012ABCD_MuMuVtx_CL.txt");
...
//  gr->GetXaxis()->SetTitle("ChiSquared/NDoF cut ");
  gr->SetTitle("B0 purity as a Function of MuMuVtx_CL cut");
  gr->GetXaxis()->SetTitle("MuMuVtx_CL cut ");
  gr->GetYaxis()->SetTitle("#frac{S}{(S+B)}");
  c_purity->SaveAs("purity_MuOnia2012ABCD_MuMuVtx_CL.pdf");
