...
//  TFile *f0 = new TFile("MuMuPiK_Jpsi_output_new_comb_B0Pt.root");
      TFile *f0 = new TFile("MuMuPiK_Jpsi_MuOnia2012ABCD_output_small_ntpls_cut_opt_trackPt.root");
  ofstream outfile;
  outfile.open("purity_MuOnia2012ABCD_trackPt.txt");
...
//  int n_gr = 23;
  double x[40],y[40],x_err[40],y_err[40];
...
//	histo[j]->Draw();
  ...
  dh[j]->plotOn(frame) ;
      ...
      RooRealVar m("m","m",5.279,5.278,5.2795); //5.279,5.25,5.3099
      RooRealVar s("s","s",0.0109,0.0101,0.012); //0.0105,0.005,0.012
      RooRealVar s2("s2","s2",0.02,0.010,0.028); // 0.03,0.01,0.039
      RooGaussian g1("g1","gaussian1",Bzero_mass,m,s);
      RooGaussian g2("g2","gaussian2",Bzero_mass,m,s2);
      RooRealVar frac("frac","frac",0.5,0.1,0.6); //0.5,0.0,1.0 //0.4,0.35,0.45
...
//  gr->GetXaxis()->SetTitle("ChiSquared/NDoF cut ");
  ...
  c_purity->SaveAs("purity_MuOnia2012ABCD_trackPt.pdf");
