...
using namespace RooFit ;
B0_fit_cut_opt_B0Vtx_CL()
{ // main
//  TCanvas* c = new TCanvas("c","test",570,400);
//  TFile *f0 = new TFile("psi2spipi_cut_opt_trial.root");

  TFile *f0 = new TFile("MuMuPiK_Jpsi_MuOnia2012ABCD_output_small_ntpls_cut_opt_B0Vtx_CL.root");
  ofstream outfile;
  outfile.open("purity_MuOnia2012ABCD_B0Vtx_CL.txt");
...
//	histo[j]->Draw();
  ...
  dh[j]->plotOn(frame) ;
      ...
      RooRealVar s2("s2","s2",0.02,0.0105,0.021); // 0.03,0.01,0.039
...
//  gr->GetXaxis()->SetTitle("ChiSquared/NDoF cut ");
  gr->SetTitle("B0 purity as a Function of B0Vtx_CL cut");
  gr->GetXaxis()->SetTitle("B0Vtx_CL cut ");
  gr->GetYaxis()->SetTitle("#frac{S}{(S+B)}");
  c_purity->SaveAs("purity_MuOnia2012ABCD_B0Vtx_CL.pdf");
