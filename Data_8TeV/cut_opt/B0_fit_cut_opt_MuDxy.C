...
using namespace RooFit ;
B0_fit_cut_opt_MuDxy()
{ // main
//  TCanvas* c = new TCanvas("c","test",570,400);
//  TFile *f0 = new TFile("psi2spipi_cut_opt_trial.root");

  TFile *f0 = new TFile("MuMuPiK_Jpsi_MuOnia2012ABCD_output_small_ntpls_cut_opt_MuDxy.root");
  ofstream outfile;
  outfile.open("purity_MuOnia2012ABCD_MuDxy.txt");
...
//  int n_gr = 23;
  double x[11],y[11],x_err[11],y_err[11];

  for (int j=1;j<n_gr;j++) { // loop over cut opt variable
...
//	histo[j]->Draw();
  ...
  dh[j]->plotOn(frame) ;
      ...
      RooRealVar s("s","s",0.0115,0.011,0.012); //0.0105,0.005,0.012
      RooRealVar s2("s2","s2",0.025,0.021,0.03); // 0.03,0.01,0.039
...
//  gr->GetXaxis()->SetTitle("ChiSquared/NDoF cut ");
  gr->SetTitle("B0 purity as a Function of Mu Dxy cut");
  gr->GetXaxis()->SetTitle("Mu Dxy cut ");
  gr->GetYaxis()->SetTitle("#frac{S}{(S+B)}");
  c_purity->SaveAs("purity_MuOnia2012ABCD_MuDxy.pdf");
