...
using namespace RooFit ;
B0_fit_cut_opt_B0CosAlphaPV()
{ // main
//  TCanvas* c = new TCanvas("c","test",570,400);
//  TFile *f0 = new TFile("psi2spipi_cut_opt_trial.root");

  TFile *f0 = new TFile("MuMuPiK_Jpsi_MuOnia2012ABCD_output_small_ntpls_cut_opt_B0CosAlphaPV.root");
  ofstream outfile;
  outfile.open("purity_MuOnia2012ABCD_B0CosAlphaPV.txt");
...
//	histo[j]->Draw();
  ...
  dh[j]->plotOn(frame) ;
      ...
      RooRealVar s("s","s",0.0105,0.010,0.012); //0.0105,0.005,0.012
      RooRealVar s2("s2","s2",0.02,0.0105,0.025); // 0.03,0.01,0.039
      RooGaussian g1("g1","gaussian1",Bzero_mass,m,s);
      RooGaussian g2("g2","gaussian2",Bzero_mass,m,s2);
      RooRealVar frac("frac","frac",0.38,0.0,0.5); //0.5,0.0,1.0 //0.4,0.35,0.45
...
//      RooChebychev cheb("cheb","BG PDF for DeltaM",Bzero_mass,RooArgList(c0,c1,c2));
      ...
      RooRealVar nsig("nsig","nsig",1000000,200000,1500000); // 10000,100,15000
      RooRealVar nbkg("nbkg","nbkg",600000,300000,5000000); // 3000,100,50000
...
//  gr->GetXaxis()->SetTitle("ChiSquared/NDoF cut ");
  gr->SetTitle("B0 purity as a Function of B0CosAlphaPV cut");
  gr->GetXaxis()->SetTitle("B0CosAlphaPV cut ");
  gr->GetYaxis()->SetTitle("#frac{S}{(S+B)}");
  c_purity->SaveAs("purity_MuOnia2012ABCD_B0CosAlphaPV.pdf");
