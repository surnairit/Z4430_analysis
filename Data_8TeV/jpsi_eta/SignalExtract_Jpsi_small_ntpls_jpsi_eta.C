#define SignalExtract_Jpsi_small_ntpls_jpsi_eta_cxx
// The class definition in SignalExtract_Jpsi_small_ntpls_jpsi_eta.h has been generated automatically
...
//
// Root > T->Process("SignalExtract_Jpsi_small_ntpls_jpsi_eta.C")
// Root > T->Process("SignalExtract_Jpsi_small_ntpls_jpsi_eta.C","some options")
// Root > T->Process("SignalExtract_Jpsi_small_ntpls_jpsi_eta.C+")
//

#include "SignalExtract_Jpsi_small_ntpls_jpsi_eta.h"
#include <TH2.h>
#include <TStyle.h>
...
void SignalExtract_Jpsi_small_ntpls_jpsi_eta::Begin(TTree * /*tree*/)
{
...
void SignalExtract_Jpsi_small_ntpls_jpsi_eta::SlaveBegin(TTree * /*tree*/)
{
   ...
   TString option = GetOption();
    ...
    OutFile = new TProofOutputFile( "MuMuPiK_Jpsi_MuOnia2012ABCD_output_small_ntpls_jpsi_eta.root" );
...
//    _nt = new TNtupleD("_nt","_nt","B0_mass:KPi_mass:Jpsi_mass:JpsiPi_mass:B0_3mom:KPi_3mom:Jpsi_3mom:K_3mom:Pi_3mom:Theta_Kstar:Theta_Jpsi:Phi:evtNum:runNum:lumiNum"); // output ntuple
  ...
  //
    ...
    char histname_sel[100];
    char histname2_sel[100];
    
    for (int j=0;j<21;j++) { // histname loop over eta bins
       
        double m=j;
        double etarange = -2.1+m*0.2;
        sprintf(histname,"hjpsiMass_eta_bin%i",j);
        sprintf(histname2,"hjpsiMass_eta_bin%dto%d;m(J/#Psi)[GeV]",etarange,etarange+0.2);
        hjpsiMass_eta_bin[j] = new TH1F(histname,histname2, 380, 2.90, 3.28);
        
        sprintf(histname_sel,"hjpsiMass_eta_bin_sel%i",j);
        sprintf(histname2_sel,"hjpsiMass_eta_bin_sel%dto%d;m(J/#Psi)[GeV]",etarange,etarange+0.2);
        hjpsiMass_eta_bin_sel[j] = new TH1F(histname_sel,histname2_sel, 380, 2.90, 3.28);

        
       
        
    }// histname loop over eta bins

}

Bool_t SignalExtract_Jpsi_small_ntpls_jpsi_eta::Process(Long64_t entry)
{
   // The Process() function is called for each entry in the tree (or possibly
   // keyed object in the case of PROOF) to be processed. The entry argument
   // specifies which entry in the currently loaded tree is to be processed.
   // It can be passed to either SignalExtract_Jpsi_small_ntpls_jpsi_eta::GetEntry() or TBranch::GetEntry()
  ...
  ) // cuts tight
   {
     ...
     hjpsiEta_sel->Fill(jpsip4.Eta());
       ...
       for (double k=0;k<21;k++){ // jpsi eta bin
           ...
           if (jpsip4.Eta()>=bin && jpsip4.Eta()<bin+0.2 ) { // cut imp cond
               int l=k;
               hjpsiMass_eta_bin_sel[l]->Fill(jpsip4.M());
               } // cut imp cond
           } // jpsi eta bin
   }
...
void SignalExtract_Jpsi_small_ntpls_jpsi_eta::SlaveTerminate()
{
...
void SignalExtract_Jpsi_small_ntpls_jpsi_eta::Terminate()
{
