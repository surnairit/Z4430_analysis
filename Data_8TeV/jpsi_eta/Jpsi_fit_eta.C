...
{ // main
    ...
    ofstream outfile;
//    outfile.open("Jpsi_Mass_etabins_MuOnia2012ABCD.txt");
    outfile.open("Jpsi_Mass_etabins_MuOnia2012ABCD_sel.txt");
  ...
  for (int j=0;j<n_gr;j++) { // loop over eta bins
...
//      sprintf(histname,"hjpsiMass_eta_bin%i",j);
      sprintf(histname,"hjpsiMass_eta_bin_sel%i",j);
...
//        dh[j]->plotOn(frame, Binning(20)) ;
      ...
      RooRealVar ng1("ng1","ng1",400000,1000,1000000); // 10000,100,15000
      RooRealVar ng2("ng2","ng2",400000,1000,1000000); // 3000,100,50000
