...
{ // main
  ...
  TFile *f0 = new TFile("MuMuPiK_Jpsi_MuOnia2012ABCD_output_small_ntpls_finalcuts_3.root");
  ofstream outfile;
  outfile.open("purity_Jpsi_MuOnia2012ABCD_test.txt");
  outfile<<"cut_val	nsig	nsig_err	nbkg	nbkg_err	S/(S+B) 	sig_err"<<endl;


//  RooRealVar Bzero_mass("Bzero_mass","M (GeV/c^{2})",5.15,5.45) ;
  RooRealVar Bzero_mass("Bzero_mass","M (GeV/c^{2})",5.10,5.46) ;
...
//	histo[j]=(TH1F*)f0->Get("hjpsiKPiMassSelAltZoomB0pt18Trackpt2p5_nB0eq1");
//	histo[j]=(TH1F*)f0->Get("hjpsiKPiMassSelAltZoomTrackpt2p5_nB0eq1");
    histo[j]=(TH1F*)f0->Get("hjpsiKPiMassSelAltZoom_nB01");
//   histo[j]=(TH1F*)f0->Get("hjpsiKPiMassSelAltZoomTrackpt4p3_nB0eq1");
//   histo[j]=(TH1F*)f0->Get("hjpsiKPiMassSelAltZoomB0pt18Trackpt4p3_nB0eq1");
...
//	histo[j]->Draw();
  ...
  RooRealVar s("s","s",0.0105,0.01,0.012);
//  RooRealVar s("s","s",0.01,0.001,0.035);
  RooRealVar s2("s2","s2",0.011,0.0105,0.025);
...
//  RooChebychev cheb("cheb","BG PDF for DeltaM",Bzero_mass , cs);
    ...
    RooChebychev cheb("cheb","BG PDF for DeltaM",Bzero_mass,RooArgList(c0,c1,c2,c3));
...
//  Bzero_mass.setRange("peak",fitted_mean-2.0*fitted_sigmalarge,fitted_mean+2.0*fitted_sigmalarge);
    Bzero_mass.setRange("peak",fitted_mean-1.0*fitted_sigmalarge,fitted_mean+1.0*fitted_sigmalarge);
//    Bzero_mass.setRange("sideband1",5.16,5.22);
//    Bzero_mass.setRange("sideband2",5.34,5.40);
//    Bzero_mass.setRange("peakDalitz",5.22,5.34);
    
//    Bzero_mass.setRange("peak",5.259,5.299); //
    Bzero_mass.setRange("sideband1",5.209,5.229); //5.10,5.18
    Bzero_mass.setRange("sideband2",5.329,5.349); //5.38,5.46
    Bzero_mass.setRange("peakDalitz",5.259,5.299); //5.259,5.299
  ...
  double sig_error = purity*sqrt(delsbys2+ (dels2plusdelb2/splusb2) );
    ...
    RooAbsReal* peakDalitzBkgN = cheb.createIntegral(Bzero_mass,Bzero_mass,"peakDalitz");
    double bkg_integral_peakDalitz =  peakDalitzBkgN->getVal();
    
    RooAbsReal* sideband1BkgN = cheb.createIntegral(Bzero_mass,Bzero_mass,"sideband1");
    double bkg_integral_sideband1 =  sideband1BkgN->getVal();
    
    RooAbsReal* sideband2BkgN = cheb.createIntegral(Bzero_mass,Bzero_mass,"sideband2");
    double bkg_integral_sideband2 =  sideband1BkgN->getVal();
    
    cout<<"peak bkg : " << bkg_integral_peakDalitz*nbkg_val_abs << "  sideband bkg : "<<(bkg_integral_sideband2+bkg_integral_sideband1)* nbkg_val_abs<< endl;
