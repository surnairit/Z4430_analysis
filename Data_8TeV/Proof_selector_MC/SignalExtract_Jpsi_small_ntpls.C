...
//   OutFile = new TProofOutputFile( "MuMuPiK_Jpsi_MuOnia2012B_output_small_ntpls.root" );
     OutFile = new TProofOutputFile( "MuMuPiK_Jpsi_MuOnia2012ABC_output_small_ntpls.root" );
...
// nB0 = 1 variables
    ...
    hjpsiPiMass2_noKstar_nB01    = new TH1F("hjpsiPiMass2_noKstar","hjpsiPiMass2_noKstar",1700,9,26);
...
// nB0 = 1 variables
    ...
    hjpsiKPiMass_noKstar 	= new TH1F("hjpsiKPiMass_noKstar","hjpsiKPiMassSel no Kstar", 90, 5.15, 5.45);
...
//    hDalitz_sideband_bin4   = new TH2F("hDalitz_sideband_bin4","hDalitz_sideband_bin4",100,0,6,425,9,26);
    ...
    hjpsiKPiMassNoPhi 		= new TH1F("hjpsiKPiMassNoPhi","hjpsiKPiMass No Phi to KK ", 90, 5.15, 5.45);
    hjpsiKPiMassBsTail 		= new TH1F("hjpsiKPiMassBsTail","hjpsiKPiMass in Bs Tail region ", 90, 5.15, 5.45);
    hjpsiKPiMassBs          = new TH1F("hjpsiKPiMassBs","hjpsiKPiMass in Bs signal region ", 90, 5.15, 5.45);

    hjpsiKPiMassBaseSelAlt 	= new TH1F("hjpsiKPiMassBaseSelAlt"," MuMuKPiMass after basic selection",10000, 0, 100);

    hjpsiKPiMassSelAltZoomB0pt 		= new TH1F("hjpsiKPiMassSelAltZoomB0pt","hjpsiKPiMassSel ALTERNATIVE ZOOM B0pt >19 GeV", 90, 5.15, 5.45);
    hjpsiKPiMassSelAltZoomJpsi09 	= new TH1F("hjpsiKPiMassSelAltZoomJpsi09","hjpsiKPiMassSel ALTERNATIVE ZOOM JpsiMass <0.09", 90, 5.15, 5.45);
    hjpsiKPiMassSelAltZoomJpsi06 	= new TH1F("hjpsiKPiMassSelAltZoomJpsi06","hjpsiKPiMassSel ALTERNATIVE ZOOM JpsiMass <0.06", 90, 5.15, 5.45);
    hjpsiKPiMassSelAltZoomTrackpt 	= new TH1F("hjpsiKPiMassSelAltZoomTrackpt","hjpsiKPiMassSel ALTERNATIVE ZOOM Trackpt >2.2 GeV", 90, 5.15, 5.45);
    hjpsiKPiMassSelAltZoomAllCuts	= new TH1F("hjpsiKPiMassSelAltZoomAllCuts","hjpsiKPiMassSelAltZoomAllCuts",90, 5.15, 5.45);
...
//    hjpsiKPiMassBaseSelAlt->Fill( B0MassAlt );
  ...
  B0p4_KPiExchanged = jpsip4 + pip4_exchanged + kp4_exchanged;
        ...
        //double
        theta_Jpsi = GetThetaMuMu(B0p4, jpsip4, muP_p4, beam_energy, jpsi_mass, muon_mass);
        //double
...
//       if ( (*B0Vtx_CL)[myB0Idx] > 0.03 ) { // b0 vertex condition
	...
	if(B0_Pt > 19.0) { // B0 pt hard cut
	hjpsiKPiMassSelAltZoomB0pt->Fill( B0MassAlt );
	} // B0 pt hard cut
	if(fabs( (*MuMuMass)[jpsi_index] - jpsi_mass ) < 0.09) { // Jpsi window < 0.09
	hjpsiKPiMassSelAltZoomJpsi09->Fill( B0MassAlt );
	} // Jpsi window < 0.09
	if(fabs( (*MuMuMass)[jpsi_index] - jpsi_mass ) < 0.06) { // Jpsi window < 0.06
	hjpsiKPiMassSelAltZoomJpsi06->Fill( B0MassAlt );
	} // Jpsi window < 0.06
	if(pion_fromB0_Pt > 2.2 && kaon_fromB0_Pt > 2.2) { // track pt hard cut
	hjpsiKPiMassSelAltZoomTrackpt->Fill( B0MassAlt );
	} // track pt hard cut

	if(   B0_Pt > 19.0 
	   && fabs( (*MuMuMass)[jpsi_index] - jpsi_mass ) < 0.06 
	   && pion_fromB0_Pt > 2.2 && kaon_fromB0_Pt > 2.2 ) { // All hard cuts
	hjpsiKPiMassSelAltZoomAllCuts->Fill( B0MassAlt );
	} // all hard cuts
...
// nB0 = 1 variables
      ...
      hjpsiKPiMassSelAltZoomB0pt->Write();
      hjpsiKPiMassSelAltZoomJpsi09->Write();
      hjpsiKPiMassSelAltZoomJpsi06->Write();
      hjpsiKPiMassSelAltZoomTrackpt->Write();
      hjpsiKPiMassSelAltZoomAllCuts->Write();
