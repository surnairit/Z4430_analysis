...
MuMuPiKPAT::MuMuPiKPAT(const edm::ParameterSet& iConfig) :
  ...
  doMuMuKPiMassConst( iConfig.getUntrackedParameter<bool>("DoMuMuKPiMassConstraint", true) ), //nsmod
  noMassConstFit( iConfig.getUntrackedParameter<bool>("NoMassConstFit", true) ), //nsmod
...
//m_dEdxDiscrimTag_kaon(m_dEdxDiscrimTag, std::string("dedxHarmonic2Kaon"))), // not working
  ...
  nMu(0), nMuMu(0), nB0(0), 
  nB0_pre0(0), nB0_pre1(0), nB0_pre2(0), nB0_pre3(0), nB0_pre4(0), nB0_pre5(0), nB0_pre6(0), nB0_pre7(0), nB0_pre8(0), nB0_pre9(0), nB0_pre10(0), nB0_pre11(0), nB0_pre12(0), nB0_pre13(0), nB0_pre14(0),  nB0massconst_notvalid(0), // nsmod
  ...
  MCpionCh(0), MCkaonCh(0), MCkaonStarCh(0), MCkaonStarId(0), //nsmod
...
*/
  ...
  // Muons and pions/tracks after B0 Cand fit with B0 mass constraint  // nsmod
  B0MassConstOK(0),
...
/*
  // Muons and pions/tracks after B0 Cand fit with Jpsi mass constraint // nsmod
  mu1Px_MuMuPiK_JpsiMass(0), mu1Py_MuMuPiK_JpsiMass(0), mu1Pz_MuMuPiK_JpsiMass(0), mu1E_MuMuPiK_JpsiMass(0),
  mu2Px_MuMuPiK_JpsiMass(0), mu2Py_MuMuPiK_JpsiMass(0), mu2Pz_MuMuPiK_JpsiMass(0), mu2E_MuMuPiK_JpsiMass(0),
  piPx_MuMuPiK_JpsiMass(0), piPy_MuMuPiK_JpsiMass(0), piPz_MuMuPiK_JpsiMass(0), piE_MuMuPiK_JpsiMass(0),
  kPx_MuMuPiK_JpsiMass(0), kPy_MuMuPiK_JpsiMass(0), kPz_MuMuPiK_JpsiMass(0), kE_MuMuPiK_JpsiMass(0),

    // Muons and pions/tracks after B0 Cand fit with no mass constraint // nsmod
  mu1Px_MuMuPiK_NoMass(0), mu1Py_MuMuPiK_NoMass(0), mu1Pz_MuMuPiK_NoMass(0), mu1E_MuMuPiK_NoMass(0),
  mu2Px_MuMuPiK_NoMass(0), mu2Py_MuMuPiK_NoMass(0), mu2Pz_MuMuPiK_NoMass(0), mu2E_MuMuPiK_NoMass(0),
  piPx_MuMuPiK_NoMass(0), piPy_MuMuPiK_NoMass(0), piPz_MuMuPiK_NoMass(0), piE_MuMuPiK_NoMass(0),
  kPx_MuMuPiK_NoMass(0), kPy_MuMuPiK_NoMass(0), kPz_MuMuPiK_NoMass(0), kE_MuMuPiK_NoMass(0),

  // B0 mass const relative change // nsmod
  mu1Px_MuMuPiK_B0Mass_rel(0), mu1Py_MuMuPiK_B0Mass_rel(0), mu1Pz_MuMuPiK_B0Mass_rel(0), mu1E_MuMuPiK_B0Mass_rel(0),
  mu2Px_MuMuPiK_B0Mass_rel(0), mu2Py_MuMuPiK_B0Mass_rel(0), mu2Pz_MuMuPiK_B0Mass_rel(0), mu2E_MuMuPiK_B0Mass_rel(0),
  piPx_MuMuPiK_B0Mass_rel(0), piPy_MuMuPiK_B0Mass_rel(0), piPz_MuMuPiK_B0Mass_rel(0), piE_MuMuPiK_B0Mass_rel(0),
  kPx_MuMuPiK_B0Mass_rel(0), kPy_MuMuPiK_B0Mass_rel(0), kPz_MuMuPiK_B0Mass_rel(0), kE_MuMuPiK_B0Mass_rel(0),

  // Jpsi mass const relative change // nsmod
  mu1Px_MuMuPiK_JpsiMass_rel(0), mu1Py_MuMuPiK_JpsiMass_rel(0), mu1Pz_MuMuPiK_JpsiMass_rel(0), mu1E_MuMuPiK_JpsiMass_rel(0),
  mu2Px_MuMuPiK_JpsiMass_rel(0), mu2Py_MuMuPiK_JpsiMass_rel(0), mu2Pz_MuMuPiK_JpsiMass_rel(0), mu2E_MuMuPiK_JpsiMass_rel(0),
  piPx_MuMuPiK_JpsiMass_rel(0), piPy_MuMuPiK_JpsiMass_rel(0), piPz_MuMuPiK_JpsiMass_rel(0), piE_MuMuPiK_JpsiMass_rel(0),
  kPx_MuMuPiK_JpsiMass_rel(0), kPy_MuMuPiK_JpsiMass_rel(0), kPz_MuMuPiK_JpsiMass_rel(0), kE_MuMuPiK_JpsiMass_rel(0),
*/
...
*/ // nsmod
   ...
   // iEvent.getByLabel("genParticles", genParticles);
    iEvent.getByLabel(inputGEN_, genParticles); //nsmod 
    ...
    int pionCh=10, kaonCh=10, kaonStarCh=10, kaonStarId=0 ; //nsmod
	...
	if ( abs(pdgid) == MCMother  &&  dauNum == MCDaughtersN ) {
	  ...
	  for (int j=0; j<dauNum; ++j) {
	    ...
	    else if ( dauNum > 2  &&  abs(dau->pdgId()) == MCDaughterID[2]  &&  !thirdDau ) { // check if one of B0 daughters is a pion (check on array length done above) //nsmod
              thirdDau = true;
		...
		dauOK[j] = true;
	     // } else { // check if one of B0 daughters is a K*0(892) which decayed to K+ pi-
              } else { // check if one of B0 daughters is a K which decayed to K+ pi- //nsmod
	  ...
	  if ( allDauOk ) {
	    ...
	    MCkaonPx->push_back(kaonPx); MCkaonPy->push_back(kaonPy); MCkaonPz->push_back(kaonPz);
            MCkaonStarId->push_back(kaonStarId); //nsmod
...
*/    //nsmod 
	...
	cout << "Too many Muons: " << thePATMuonHandle->size() << ", and Tracks: " << thePATTrackHandle->size() << endl;
        ...
        if (Debug_) cout << "PAT muon size : " << thePATMuonHandle->size() << "  Trigger ? : "<< hasRequestedTrigger << endl; // nsmod
...
//            }
//            if (noMassConstFit) { //nsmod
//              B0VertexNomassConstFitTree = B0FitterNoMassConst.fit( b0Daughters );
//            }
/*
		    else {
...
*/
		    ...
		    */
//		    if ( !B0VertexFitTree->isValid() ) // nsmod
//            if ( !B0VertexFitTree->isValid() || !B0VertexB0massConstFitTree ->isValid() || !B0VertexNomassConstFitTree->isValid() )
...
//            if ( B0Cand_vertex_fromMCFit->vertexIsValid() ) { // nsmod temp
...
/*
            // after B0 Cand fit with Jpsi mass constraint // nsmod // only used for JpsiMass const vs B0Mass const study, separate index
		    B0VertexFitTree->movePointerToTheFirstChild();
		    RefCountedKinematicParticle mu1_MuMuPiK_JpsiMass = B0VertexFitTree->currentParticle();
		    B0VertexFitTree->movePointerToTheNextChild();
		    RefCountedKinematicParticle mu2_MuMuPiK_JpsiMass = B0VertexFitTree->currentParticle();
		    B0VertexFitTree->movePointerToTheNextChild();
		    RefCountedKinematicParticle pi_MuMuPiK_JpsiMass = B0VertexFitTree->currentParticle();
		    B0VertexFitTree->movePointerToTheNextChild();
		    RefCountedKinematicParticle k_MuMuPiK_JpsiMass = B0VertexFitTree->currentParticle();



		    // muon1 after B0 Cand fit with Jpsi mass constraint // nsmod
		    mu1Px_MuMuPiK_JpsiMass_val = ( mu1_MuMuPiK_JpsiMass->currentState().globalMomentum().x() );
		    mu1Py_MuMuPiK_JpsiMass_val = ( mu1_MuMuPiK_JpsiMass->currentState().globalMomentum().y() );
		    mu1Pz_MuMuPiK_JpsiMass_val = ( mu1_MuMuPiK_JpsiMass->currentState().globalMomentum().z() );
		    mu1E_MuMuPiK_JpsiMass_val = ( mu1_MuMuPiK_JpsiMass->currentState().kinematicParameters().energy() );
		    // muon2 after B0 Cand fit with Jpsi mass constraint // nsmod
		    mu2Px_MuMuPiK_JpsiMass_val = ( mu2_MuMuPiK_JpsiMass->currentState().globalMomentum().x() );
		    mu2Py_MuMuPiK_JpsiMass_val = ( mu2_MuMuPiK_JpsiMass->currentState().globalMomentum().y() );
		    mu2Pz_MuMuPiK_JpsiMass_val = ( mu2_MuMuPiK_JpsiMass->currentState().globalMomentum().z() );
		    mu2E_MuMuPiK_JpsiMass_val = ( mu2_MuMuPiK_JpsiMass->currentState().kinematicParameters().energy() );
		    // pion after B0 Cand fit with Jpsi mass constraint // nsmod
		    piPx_MuMuPiK_JpsiMass_val = ( pi_MuMuPiK_JpsiMass->currentState().globalMomentum().x() );
		    piPy_MuMuPiK_JpsiMass_val = ( pi_MuMuPiK_JpsiMass->currentState().globalMomentum().y() );
		    piPz_MuMuPiK_JpsiMass_val = ( pi_MuMuPiK_JpsiMass->currentState().globalMomentum().z() );
		    piE_MuMuPiK_JpsiMass_val = ( pi_MuMuPiK_JpsiMass->currentState().kinematicParameters().energy() );
		    // kaon after B0 Cand fit with Jpsi mass constraint // nsmod
		    kPx_MuMuPiK_JpsiMass_val = ( k_MuMuPiK_JpsiMass->currentState().globalMomentum().x() );
		    kPy_MuMuPiK_JpsiMass_val = ( k_MuMuPiK_JpsiMass->currentState().globalMomentum().y() );
		    kPz_MuMuPiK_JpsiMass_val = ( k_MuMuPiK_JpsiMass->currentState().globalMomentum().z() );
		    kE_MuMuPiK_JpsiMass_val = ( k_MuMuPiK_JpsiMass->currentState().kinematicParameters().energy() );
*/
            ...
            // after B0 Cand fit with B0 mass constraint // nsmod
...
//            // after B0 Cand fit with Jpsi mass constraint // nsmod
//		    B0VertexFitTree->movePointerToTheFirstChild();
//		    RefCountedKinematicParticle mu1_MuMuPiK_JpsiMass = B0VertexFitTree->currentParticle();
//		    B0VertexFitTree->movePointerToTheNextChild();
//		    RefCountedKinematicParticle mu2_MuMuPiK_JpsiMass = B0VertexFitTree->currentParticle();
...
/*
            // after B0 Cand fit with No mass constraint // nsmod
		    B0VertexNomassConstFitTree->movePointerToTheFirstChild();
		    RefCountedKinematicParticle mu1_MuMuPiK_NoMass = B0VertexNomassConstFitTree->currentParticle();
		    B0VertexNomassConstFitTree->movePointerToTheNextChild();
		    RefCountedKinematicParticle mu2_MuMuPiK_NoMass = B0VertexNomassConstFitTree->currentParticle();
		    B0VertexNomassConstFitTree->movePointerToTheNextChild();
		    RefCountedKinematicParticle pi_MuMuPiK_NoMass = B0VertexNomassConstFitTree->currentParticle();
		    B0VertexNomassConstFitTree->movePointerToTheNextChild();
		    RefCountedKinematicParticle k_MuMuPiK_NoMass = B0VertexNomassConstFitTree->currentParticle();
*/
...
//
//		    // muon1 after B0 Cand fit with No mass constraint // nsmod
//		    mu1Px_MuMuPiK_NoMass->push_back( mu1_MuMuPiK_NoMass->currentState().globalMomentum().x() );
//		    mu1Py_MuMuPiK_NoMass->push_back( mu1_MuMuPiK_NoMass->currentState().globalMomentum().y() );
//		    mu1Pz_MuMuPiK_NoMass->push_back( mu1_MuMuPiK_NoMass->currentState().globalMomentum().z() );
//		    mu1E_MuMuPiK_NoMass->push_back( mu1_MuMuPiK_NoMass->currentState().kinematicParameters().energy() );
//		    // muon2 after B0 Cand fit with No mass constraint // nsmod
//		    mu2Px_MuMuPiK_NoMass->push_back( mu2_MuMuPiK_NoMass->currentState().globalMomentum().x() );
//		    mu2Py_MuMuPiK_NoMass->push_back( mu2_MuMuPiK_NoMass->currentState().globalMomentum().y() );
//		    mu2Pz_MuMuPiK_NoMass->push_back( mu2_MuMuPiK_NoMass->currentState().globalMomentum().z() );
//		    mu2E_MuMuPiK_NoMass->push_back( mu2_MuMuPiK_NoMass->currentState().kinematicParameters().energy() );
//		    // pion after B0 Cand fit with No mass constraint // nsmod
//		    piPx_MuMuPiK_NoMass->push_back( pi_MuMuPiK_NoMass->currentState().globalMomentum().x() );
//		    piPy_MuMuPiK_NoMass->push_back( pi_MuMuPiK_NoMass->currentState().globalMomentum().y() );
//		    piPz_MuMuPiK_NoMass->push_back( pi_MuMuPiK_NoMass->currentState().globalMomentum().z() );
//		    piE_MuMuPiK_NoMass->push_back( pi_MuMuPiK_NoMass->currentState().kinematicParameters().energy() );
//		    // kaon after B0 Cand fit with No mass constraint // nsmod
//		    kPx_MuMuPiK_NoMass->push_back( k_MuMuPiK_NoMass->currentState().globalMomentum().x() );
//		    kPy_MuMuPiK_NoMass->push_back( k_MuMuPiK_NoMass->currentState().globalMomentum().y() );
//		    kPz_MuMuPiK_NoMass->push_back( k_MuMuPiK_NoMass->currentState().globalMomentum().z() );
//		    kE_MuMuPiK_NoMass->push_back( k_MuMuPiK_NoMass->currentState().kinematicParameters().energy() );
...
/*
		    ...
		    mu2Pz_MuMuPiK_JpsiMass_rel->push_back(( mu2_MuMuPiK_JpsiMass->currentState().globalMomentum().z() - rmu2->pz() )/ rmu2->pz());
...
*/
        ...
        } //  B0 mass const valid // nsmod



		    //
		  
		    if ( !B0Cand_vertex_fromMCFit->vertexIsValid() ) // nsmod
		      continue ; nB0_pre11++ ;
		    ...
		    b0Mass->push_back( B0Cand_fromMCFit->currentState().mass()) ; // nsmod temp
  ...
  */
  ...
  // muons and tracks after B0 Cand fit B0 mass const // nsmod
  B0MassConstOK->clear();
...
/*
  // muons and tracks after B0 Cand fit Jpsi mass const // nsmod
  mu1Px_MuMuPiK_JpsiMass->clear(); mu1Py_MuMuPiK_JpsiMass->clear(); mu1Pz_MuMuPiK_JpsiMass->clear(); mu1E_MuMuPiK_JpsiMass->clear();
  mu2Px_MuMuPiK_JpsiMass->clear(); mu2Py_MuMuPiK_JpsiMass->clear(); mu2Pz_MuMuPiK_JpsiMass->clear(); mu2E_MuMuPiK_JpsiMass->clear();
  piPx_MuMuPiK_JpsiMass->clear(); piPy_MuMuPiK_JpsiMass->clear(); piPz_MuMuPiK_JpsiMass->clear(); piE_MuMuPiK_JpsiMass->clear();
  kPx_MuMuPiK_JpsiMass->clear(); kPy_MuMuPiK_JpsiMass->clear(); kPz_MuMuPiK_JpsiMass->clear(); kE_MuMuPiK_JpsiMass->clear();

  // muons and tracks after B0 Cand fit No mass const // nsmod
  mu1Px_MuMuPiK_NoMass->clear(); mu1Py_MuMuPiK_NoMass->clear(); mu1Pz_MuMuPiK_NoMass->clear(); mu1E_MuMuPiK_NoMass->clear();
  mu2Px_MuMuPiK_NoMass->clear(); mu2Py_MuMuPiK_NoMass->clear(); mu2Pz_MuMuPiK_NoMass->clear(); mu2E_MuMuPiK_NoMass->clear();
  piPx_MuMuPiK_NoMass->clear(); piPy_MuMuPiK_NoMass->clear(); piPz_MuMuPiK_NoMass->clear(); piE_MuMuPiK_NoMass->clear();
  kPx_MuMuPiK_NoMass->clear(); kPy_MuMuPiK_NoMass->clear(); kPz_MuMuPiK_NoMass->clear(); kE_MuMuPiK_NoMass->clear();


  // B0 mass const relative change // nsmod
  mu1Px_MuMuPiK_B0Mass_rel->clear(); mu1Py_MuMuPiK_B0Mass_rel->clear(); mu1Pz_MuMuPiK_B0Mass_rel->clear(); mu1E_MuMuPiK_B0Mass_rel->clear();
  mu2Px_MuMuPiK_B0Mass_rel->clear(); mu2Py_MuMuPiK_B0Mass_rel->clear(); mu2Pz_MuMuPiK_B0Mass_rel->clear(); mu2E_MuMuPiK_B0Mass_rel->clear();
  piPx_MuMuPiK_B0Mass_rel->clear(); piPy_MuMuPiK_B0Mass_rel->clear(); piPz_MuMuPiK_B0Mass_rel->clear(); piE_MuMuPiK_B0Mass_rel->clear();
  kPx_MuMuPiK_B0Mass_rel->clear(); kPy_MuMuPiK_B0Mass_rel->clear(); kPz_MuMuPiK_B0Mass_rel->clear(); kE_MuMuPiK_B0Mass_rel->clear();

  // Jpsi mass const relative change // nsmod
  mu1Px_MuMuPiK_JpsiMass_rel->clear(); mu1Py_MuMuPiK_JpsiMass_rel->clear(); mu1Pz_MuMuPiK_JpsiMass_rel->clear(); mu1E_MuMuPiK_JpsiMass_rel->clear();
  mu2Px_MuMuPiK_JpsiMass_rel->clear(); mu2Py_MuMuPiK_JpsiMass_rel->clear(); mu2Pz_MuMuPiK_JpsiMass_rel->clear(); mu2E_MuMuPiK_JpsiMass_rel->clear();
  piPx_MuMuPiK_JpsiMass_rel->clear(); piPy_MuMuPiK_JpsiMass_rel->clear(); piPz_MuMuPiK_JpsiMass_rel->clear(); piE_MuMuPiK_JpsiMass_rel->clear();
  kPx_MuMuPiK_JpsiMass_rel->clear(); kPy_MuMuPiK_JpsiMass_rel->clear(); kPz_MuMuPiK_JpsiMass_rel->clear(); kE_MuMuPiK_JpsiMass_rel->clear();
*/
...
void MuMuPiKPAT::beginJob()
{
  ...
  if (doGEN) {
    ...
    Z_One_Tree_->Branch("MCkaonStarId",&MCkaonStarId); //nsmod
...
/*
  // muons and tracks after B0 Cand fit Jpsi mass const // nsmod
  Z_One_Tree_->Branch("Muon1Px_MuMuPiK_JpsiMass", &mu1Px_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("Muon1Py_MuMuPiK_JpsiMass", &mu1Py_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("Muon1Pz_MuMuPiK_JpsiMass", &mu1Pz_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("Muon1E_MuMuPiK_JpsiMass", &mu1E_MuMuPiK_JpsiMass);
  //
  Z_One_Tree_->Branch("Muon2Px_MuMuPiK_JpsiMass", &mu2Px_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("Muon2Py_MuMuPiK_JpsiMass", &mu2Py_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("Muon2Pz_MuMuPiK_JpsiMass", &mu2Pz_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("Muon2E_MuMuPiK_JpsiMass", &mu2E_MuMuPiK_JpsiMass);
  //
  Z_One_Tree_->Branch("PionPx_MuMuPiK_JpsiMass", &piPx_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("PionPy_MuMuPiK_JpsiMass", &piPy_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("PionPz_MuMuPiK_JpsiMass", &piPz_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("PionE_MuMuPiK_JpsiMass", &piE_MuMuPiK_JpsiMass);
  //
  Z_One_Tree_->Branch("KaonPx_MuMuPiK_JpsiMass", &kPx_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("KaonPy_MuMuPiK_JpsiMass", &kPy_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("KaonPz_MuMuPiK_JpsiMass", &kPz_MuMuPiK_JpsiMass);
  Z_One_Tree_->Branch("KaonE_MuMuPiK_JpsiMass", &kE_MuMuPiK_JpsiMass);


  // muons and tracks after B0 Cand fit No mass const // nsmod
  Z_One_Tree_->Branch("Muon1Px_MuMuPiK_NoMass", &mu1Px_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("Muon1Py_MuMuPiK_NoMass", &mu1Py_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("Muon1Pz_MuMuPiK_NoMass", &mu1Pz_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("Muon1E_MuMuPiK_NoMass", &mu1E_MuMuPiK_NoMass);
  //
  Z_One_Tree_->Branch("Muon2Px_MuMuPiK_NoMass", &mu2Px_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("Muon2Py_MuMuPiK_NoMass", &mu2Py_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("Muon2Pz_MuMuPiK_NoMass", &mu2Pz_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("Muon2E_MuMuPiK_NoMass", &mu2E_MuMuPiK_NoMass);
  //
  Z_One_Tree_->Branch("PionPx_MuMuPiK_NoMass", &piPx_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("PionPy_MuMuPiK_NoMass", &piPy_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("PionPz_MuMuPiK_NoMass", &piPz_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("PionE_MuMuPiK_NoMass", &piE_MuMuPiK_NoMass);
  //
  Z_One_Tree_->Branch("KaonPx_MuMuPiK_NoMass", &kPx_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("KaonPy_MuMuPiK_NoMass", &kPy_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("KaonPz_MuMuPiK_NoMass", &kPz_MuMuPiK_NoMass);
  Z_One_Tree_->Branch("KaonE_MuMuPiK_NoMass", &kE_MuMuPiK_NoMass);


  // B0 mass const relative change // nsmod
  Z_One_Tree_->Branch("Muon1Px_MuMuPiK_B0Mass_rel", &mu1Px_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("Muon1Py_MuMuPiK_B0Mass_rel", &mu1Py_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("Muon1Pz_MuMuPiK_B0Mass_rel", &mu1Pz_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("Muon1E_MuMuPiK_B0Mass_rel", &mu1E_MuMuPiK_B0Mass_rel);
  //
  Z_One_Tree_->Branch("Muon2Px_MuMuPiK_B0Mass_rel", &mu2Px_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("Muon2Py_MuMuPiK_B0Mass_rel", &mu2Py_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("Muon2Pz_MuMuPiK_B0Mass_rel", &mu2Pz_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("Muon2E_MuMuPiK_B0Mass_rel", &mu2E_MuMuPiK_B0Mass_rel);
  //
  Z_One_Tree_->Branch("PionPx_MuMuPiK_B0Mass_rel", &piPx_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("PionPy_MuMuPiK_B0Mass_rel", &piPy_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("PionPz_MuMuPiK_B0Mass_rel", &piPz_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("PionE_MuMuPiK_B0Mass_rel", &piE_MuMuPiK_B0Mass_rel);
  //
  Z_One_Tree_->Branch("KaonPx_MuMuPiK_B0Mass_rel", &kPx_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("KaonPy_MuMuPiK_B0Mass_rel", &kPy_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("KaonPz_MuMuPiK_B0Mass_rel", &kPz_MuMuPiK_B0Mass_rel);
  Z_One_Tree_->Branch("KaonE_MuMuPiK_B0Mass_rel", &kE_MuMuPiK_B0Mass_rel);


  // Jpsi mass const relative change // nsmod
  Z_One_Tree_->Branch("Muon1Px_MuMuPiK_JpsiMass_rel", &mu1Px_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("Muon1Py_MuMuPiK_JpsiMass_rel", &mu1Py_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("Muon1Pz_MuMuPiK_JpsiMass_rel", &mu1Pz_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("Muon1E_MuMuPiK_JpsiMass_rel", &mu1E_MuMuPiK_JpsiMass_rel);
  //
  Z_One_Tree_->Branch("Muon2Px_MuMuPiK_JpsiMass_rel", &mu2Px_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("Muon2Py_MuMuPiK_JpsiMass_rel", &mu2Py_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("Muon2Pz_MuMuPiK_JpsiMass_rel", &mu2Pz_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("Muon2E_MuMuPiK_JpsiMass_rel", &mu2E_MuMuPiK_JpsiMass_rel);
  //
  Z_One_Tree_->Branch("PionPx_MuMuPiK_JpsiMass_rel", &piPx_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("PionPy_MuMuPiK_JpsiMass_rel", &piPy_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("PionPz_MuMuPiK_JpsiMass_rel", &piPz_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("PionE_MuMuPiK_JpsiMass_rel", &piE_MuMuPiK_JpsiMass_rel);
  //
  Z_One_Tree_->Branch("KaonPx_MuMuPiK_JpsiMass_rel", &kPx_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("KaonPy_MuMuPiK_JpsiMass_rel", &kPy_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("KaonPz_MuMuPiK_JpsiMass_rel", &kPz_MuMuPiK_JpsiMass_rel);
  Z_One_Tree_->Branch("KaonE_MuMuPiK_JpsiMass_rel", &kE_MuMuPiK_JpsiMass_rel);
...
*/
  ...
  Z_One_Tree_->Branch("nB0massconst_notvalid",&nB0massconst_notvalid,"nB0massconst_notvalid/i"); // nsmod
